...
| Tools | 70, all read-only. Every tool carries `title`, `annotations.readOnlyHint: true`, `destructiveHint: false`, `openWorldHint: false`, and `_meta["marketcode.ai/credits"]` with its price |
...
on all 70. `openWorldHint` is false because every tool answers from
...
- [x] `title` and read-only annotations on all 70 tools; `_meta` price on each
