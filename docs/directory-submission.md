...
| Tools | 94 — 90 read-only, four that write (saved searches). Every tool carries `title` (top-level and `annotations.title`), `annotations.readOnlyHint: true`, `destructiveHint: false`, `openWorldHint: false`, and `_meta["marketcode.ai/credits"]` with its price |
...
on all 94. `openWorldHint` is false because every tool answers from
...
- [x] `title` (top-level and inside `annotations`) and read-only annotations on all 94 tools; `_meta` price on each
